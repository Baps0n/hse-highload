...
Графовое представление данных применимо для достаточно узкого круга задач и в этой связи довольно слабо развито. Крупные сервисы карт, скорее всего, используют какие-либо собственные реализации.  
...
Остальные варианты недостаточно зрелые для высоконагруженного сценария. В результате, выбор сделан в пользу плагина для PostgreSQL, поскольку он наиболее популярен и обладает всем необходимым для построения навигации функционалом.
...
flowchart TD
    ...
    subgraph DC["Дата‑центр (Москва/Новосибирск)"]
        ...
        L7_DC["L7 Balancer Pool<br/>4 узла (3+1)<br/>NGINX Ingress<br/>авторизация (JWT)"]
        
        subgraph Backends["Бэкенды"]
            Routes_Pods["Routes Pods<br/>построение маршрутов"]
            Search_Pods["Search Pods<br/>поиск организаций"]
    ...
    L4_DC --> L7_DC
    L7_DC -->|авторизация, rate limit| Redis
    L7_DC --> Search_Pods
    L7_DC --> Routes_Pods
    L7_DC --> Images_Pods

    Search_Pods -->|"чтение/запись"| Redis
    Search_Pods -->|"чтение"| Postgres_POI
    Search_Pods -->|"избранное"| Postgres_Users

    Routes_Pods -->|"чтение графа"| Postgres_Road
    Routes_Pods -->|"кэш маршрутов"| Redis
...
### Kubernetes кластер дата-центра
...
| **Бэкенды** |||||
| Search Pods | 4 / 20 | 4 / 8 ядер | 8 / 16 ГБ | – | HPA по RPS, CPU |
| Routes Pods | 4 / 20 | 4 / 8 ядер | 8 / 16 ГБ | – | Построение маршрутов |
| Images Pods | 8 / 30 | 2 / 4 ядра | 4 / 8 ГБ | – | Метаданные, прокси Ceph |
| PgBouncer (sidecar) | 1 на каждый под | 0.5 / 1 ядро | 1 / 2 ГБ | – | Пул соединений к БД |
...
### Stateful СУБД дата-центра

| Группа СУБД | Роль / компонент | Количество серверов | CPU лимит (ядра) | RAM лимит (ГБ) | Диск лимит (ТБ, тип) | Примечание |
|-------------|------------------|---------------------|----------------------|----------|----------------|-------------|
| **PostgreSQL (синхронный)** | Master (запись) | 1 | 64 | 256 | 2 NVMe | USERS, FAVORITES, CATEGORIES |
| | Sync replica | 2 | 64 | 256 | 2 NVMe | Синхронная репликация |
| | Async replica (чтение) | 3 | 64 | 256 | 2 NVMe | Балансировка read‑запросов |
| **PostgreSQL+PostGIS (шарды POI)** | 32 шарда × (master + replica) | 64 | 32 | 128 | 1 SSD | Географическое шардирование, асинхронная репликация |
| **PostgreSQL+pgRouting (граф дорог)** | Master | 1 | 64 | 256 | 2 NVMe | Статический граф, обновления офлайн |
| | Async replica | 1 | 64 | 256 | 2 NVMe | Для чтения серверами маршрутизации |
| **PostgreSQL TimescaleDB (ROAD_EVENT)** | Master | 1 | 32 | 128 | 1 SSD | События дорог, партиционирование по времени, TTL 30 дней |
| | Async replica | 1 | 32 | 128 | 1 SSD | Асинхронная реплика |
| **Redis** | 3 masters | 3 | 16 | 64 | 1 SSD | Хэш‑кольцо для кэша, сессий, rate limiting |
| | 3 replicas | 3 | 16 | 64 | 0.2 SSD | Автоматический failover |
| **Ceph** | Object storage daemon | 20 | 16 | 32 | 400 HDD | Erasure coding 4+2 |
| | Monitor + Manager | 3 | 8 | 16 | 0.2 SSD | Управление кластером |
